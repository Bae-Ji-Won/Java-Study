...
public class UserDao {
    ...
    private DataSource dataSource;  // connectionmaker -> datasource 변경

    public UserDao(){                  // 생성자를 통해 AWS DB의 makeConnection()을 오버라이딩하여 사용
        this.dataSource = new UserDaoFactory().awsDataSource();
    }
    ...
    public void jdbcContextWithStatementStatementstrategy(StatementStrategy st){
        ...
        try {                   // 예외처리
            conn = dataSource.getConnection();      // datasource를 사용하게 변경
...
//        jdbcContextWithStatementStatementstrategy(new StatementStrategy() {
//            @Override
